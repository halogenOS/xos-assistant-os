...
let
  ...
  assistant = pkgs.rustPlatform.buildRustPackage {
    ...
    # The changelog the bot serves through its changelog tool, embedded at
    # compile time. The file is regenerated by update-changelog.sh in the
    # same commit that moves the assistant-src pin, so the served text and
    # the running revision cannot drift.
    env.ASSISTANT_BUILD_CHANGELOG = builtins.readFile ./assistant-changelog.txt;
  ...
  configFile = pkgs.writeText "assistant.toml" ''
    ...
    ${lib.optionalString (cfg.contextWindow != null) ''
      context_window = ${toString cfg.contextWindow}
    ''}
...
in
{
  options.custom.assistant = {
    ...
    contextWindow = lib.mkOption {
      ...
      default = null;
      description = ''
        The configured model's context window in tokens, as its provider
        documents it. The bot's automatic compaction thresholds read this;
        unset, they stay silent and only /compact and the forced turn end
        compact.
      '';
    };
